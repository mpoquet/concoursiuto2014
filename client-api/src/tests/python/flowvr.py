...
def gameRound(session):
    ...
    print "Flood en cours..."
    planetFlood = planets[0].planetId
    for i in xrange(15*1000*1000):
        session.orderBuild(planetFlood, 1)

    print "Flood terminé, envoi !"
    session.sendOrders()


def game(ip, port):
    session = gameInit("flowvr", ip, port)
